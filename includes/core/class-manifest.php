...
/**
 * WebMCP manifest generator class.
 *
 * @package GoldtWebMCP
 */
class Manifest {
	...
	public function register_tool( $name, $config ) {
		...
		$required_fields = array( 'description', 'input_schema' );
	...
	public function generate_json( $pretty = false ) {
		...
		return wp_json_encode( $manifest, $options );
	}
	...
	public function set_metadata( $key, $value ) {
		$allowed_keys = array( 'name', 'version', 'description', 'schema_version', 'api_version' );
